...
parser.add_argument('--clinic', dest='clinic', action='store_true',
                    help='use clinical data to train model')
parser.add_argument('--acm', dest='acm', action='store_true',
                    help='use Attention Comparison Module in the model')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        if os.path.isfile(args.curriculum):
            ...
            try:
                # binary -> multi
                model = OsteoSiameseNet(32, 0.3, 2, True, args.acm, use_clinic=args.clinic, use_acm=False, return_fc=True)
                loaded_model = load_model(args.curriculum, model)
                model = tf_learning(loaded_model, 3)
            except:
                # multi -> binary
                model = OsteoSiameseNet(32, 0.3, 3, True, args.clinic, args.acm, use_clinic=args.clinic, use_acm=False, return_fc=True)
                loaded_model = load_model(args.curriculum, model)
        ...
        if args.multi:
            model = OsteoSiameseNet(32, 0.3, 3, use_w_init=True, use_clinic=args.clinic, use_acm=args.acm, return_fc=True)
        else:
            # most general
            model = OsteoSiameseNet(32, 0.3, 2, use_w_init=True, use_clinic=args.clinic, use_acm=args.acm, return_fc=True)
    ...
    train_dataset = OsteoSiameseDataset(train_list, transform, oai=False, multi=args.multi, triplet=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, TB, triplet_loss)

        # evaluate on validation set
        acc1 = validate(val_loader, model, criterion, epoch, args, TB, triplet_loss)
...
def train(train_loader, model, criterion, optimizer, epoch, args, TB, triplet_loss):
    ...
    for i, data in enumerate(train_loader):
        ...
        clinic = torch.Tensor(clinic).type(torch.FloatTensor).cuda()
        ...
        # compute output
        output, anchor_feature = model(*images, clinic)
        cls_loss = criterion(output, target)

        # Triplet Losses
        pos_left, pos_right = data['positive']
        neg_left, neg_right = data['negative']

        _, positive_feature = model(x1=pos_left.cuda(), x2=pos_right.cuda(), clinic=None)
        _, negative_feature = model(x1=neg_left.cuda(), x2=neg_right.cuda(), clinic=None)

        tp_loss = triplet_loss(anchor_feature, positive_feature, negative_feature)

        # total loss
        loss = cls_loss + tp_loss * 2
...
def validate(val_loader, model, criterion, epoch, args, TB, triplet_loss):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    progress = ProgressMeter(
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            # compute output
            output, _ = model(left_, right_, clinic, args.save_fc)
    ...
    confidence_save(names, outputs, './log/', args.version)
...
def save_checkpoint(state, is_best, version):
    filename='./log/checkpoint_{}.pth.tar'.format(version)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, './log/model_best_{}.pth.tar'.format(version))
...
def confidence_save(names, outputs, save_folder, version):
    ...
    external_result.to_csv(os.path.join(save_folder, 'siamese_dl_{}.csv'.format(version)))
